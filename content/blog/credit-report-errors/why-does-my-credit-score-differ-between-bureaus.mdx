...
notionId: ""
faq:
  - q: "Why do I have different credit scores at Equifax, Experian, and TransUnion?"
    a: "The bureaus are independent companies with separate databases. Each calculates your score from its own data — and since not every creditor reports to all three, the underlying data differs, producing different scores."
  - q: "Is a large gap between bureau scores a sign of an error?"
    a: "A small difference is normal. A large gap — especially if one score is significantly lower — can indicate an error, a collection, or a negative item that only appears at one bureau. Pull all three reports to compare."
  - q: "Why does my score change even when I haven't done anything new?"
    a: "Creditors update their reporting at different times, so your reported balances, payment statuses, and account information change throughout the month. A balance update at one bureau can shift your score before the others reflect the same change."
  - q: "Which credit score should I check before applying for a loan?"
    a: "Most mortgage lenders use FICO Score 5, 4, and 2 (one from each bureau). Auto lenders often use FICO Auto Scores. Credit card issuers commonly use FICO Score 8. Ask the specific lender which model they use before you apply."
---
